...
async function loadQuestionFromRaw(raw, onSuccess){
  ...
  try{
    ...
    saveGame();
    if(typeof onSuccess === 'function') onSuccess(question);
  }catch(e){
...
function syncRestoredBuildUi(){
  ...
  if(currentBuiltQuestion){
    syncLoadedQuestionHighlights(currentBuiltQuestion);
  }else if(qtype === 'matching' && qparams.matching_cat){
    highlightBoundary(qparams.matching_cat);
